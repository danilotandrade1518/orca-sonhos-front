...
import type { TransactionType } from '../../../../../dtos/transaction/transaction-types';

@Component({
  selector: 'os-transactions-create-page',
  imports: [
    ReactiveFormsModule,
    OsPageComponent,
    OsPageHeaderComponent,
  ...
  template: `
    <os-page variant="default" size="medium" ariaLabel="Criar nova transação">
      <os-page-header
        [title]="pageTitle()"
...
export class TransactionsCreatePage implements OnInit {
  ...
  readonly pageTitle = computed(() => 'Criar Transação');

  readonly pageSubtitle = computed(() => 'Preencha os dados para criar uma nova transação');
  ...
  ngOnInit(): void {
    const form = new FormGroup({
      ...
      type: new FormControl<TransactionType>('EXPENSE' as TransactionType, [Validators.required]),
      ...
      creditCardId: new FormControl<string>(''),
    });
