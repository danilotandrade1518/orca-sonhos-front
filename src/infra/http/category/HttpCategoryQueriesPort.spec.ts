...
import { HttpCategoryQueriesPort } from './HttpCategoryQueriesPort';

describe('HttpCategoryQueriesPort (with MSW)', () => {
  it('listCategories returns categories for budget', async () => {
    const http = new FetchHttpClient({ baseUrl: ENV.API_BASE_URL });
    const adapter = new HttpCategoryQueriesPort(http);
    ...
    const items = await adapter.listCategories({ userId, budgetId });
