...
## 📅 FASE 1: Correção de Carregamento de Recursos [Status: ⏳]
...
- [ ] Orçamento é selecionado no `BudgetSelectionService` ao inicializar componente
- [ ] `effect()` detecta quando `selectedBudgetId` muda
- [ ] `loadResources()` é chamado automaticamente quando orçamento está disponível
- [ ] Contas são carregadas via `accountState.loadAccounts()`
- [ ] Participantes são carregados via `sharingState.loadParticipants(id)`
- [ ] Polling de participantes é iniciado via `sharingState.startPolling(id)`
- [ ] Não há carregamentos duplicados (flag `resourcesLoaded` funciona)
- [ ] Recursos são carregados mesmo quando orçamentos precisam ser carregados primeiro
